...
def init_db():
    c = get_conn()
    c.execute("""CREATE TABLE IF NOT EXISTS bulletins_bia(
        id INTEGER PRIMARY KEY AUTOINCREMENT, numero_bia TEXT UNIQUE,
    ...
    c.commit(); c.close()
def gen_bia():
    c=get_conn(); n=c.execute("SELECT COUNT(*) FROM bulletins_bia").fetchone()[0]+1; c.close()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    conn=get_conn()
    nb_bia=conn.execute("SELECT COUNT(*) FROM bulletins_bia").fetchone()[0]
    cot_tot=conn.execute("SELECT COALESCE(SUM(cotisation),0) FROM bulletins_bia").fetchone()[0]
    nb_val=conn.execute("SELECT COUNT(*) FROM bulletins_bia WHERE statut='Validé'").fetchone()[0]
    conn.close()
    c1,c2,c3=st.columns(3)
...
|**Option garantie**|{st.session_state.get('f_gar','—')}|""")
        ...
        def save_bia(statut_ov=None):
            ...
            data={
                "numero_bia":gen_bia(),"date_saisie":today.isoformat(),"saisi_par":user["nom"],
                "agence":st.session_state.get("f_agence",""),"code_apporteur":st.session_state.get("f_code_appo",""),
                "nom_apporteur":st.session_state.get("f_nom_appo",""),"realisateur":st.session_state.get("f_realis",""),
                "deja_assure":st.session_state.get("f_deja","Non"),"num_ct_exist":st.session_state.get("f_num_ct",""),
                "c_titre":st.session_state.get("f_c_tit",""),"c_nom":st.session_state.get("f_c_nom","").upper().strip(),
                "c_prenom":st.session_state.get("f_c_prn","").strip(),
                "c_ddn":str(st.session_state.get("f_c_ddn",date(1985,1,1))),
                "c_lieu":st.session_state.get("f_c_lieu",""),"c_nat":st.session_state.get("f_c_nat","Béninoise"),
                "c_mat":st.session_state.get("f_c_mat",""),"c_prof":st.session_state.get("f_c_prof",""),
                "c_adr":st.session_state.get("f_c_adr",""),"c_bp":st.session_state.get("f_c_bp",""),
                "c_email":st.session_state.get("f_c_eml",""),"c_wapp":st.session_state.get("f_c_wapp",""),
                "c_tel":st.session_state.get("f_c_tel",""),"c_fixe":st.session_state.get("f_c_fixe",""),
                ...
                "code_produit":prod["code"],"produit":prod["nom"],"groupe_produit":prod["grp"],
                "cotisation":float(st.session_state.get("f_coti",0)),"cotisation_lettres":st.session_state.get("f_cotil",""),
                "periodicite":st.session_state.get("f_peri","Mensuelle"),
                "date_effet":str(st.session_state.get("f_deff",today)),
                "duree":(st.session_state.get("f_duree_mois_v",12) if is_vigninou
                         else int(st.session_state.get("f_duree",10))),
                "option_gar":st.session_state.get("f_gar","Sans garantie décès"),
                "mode_reglement":st.session_state.get("f_mode",""),"mode_ref":st.session_state.get("f_mref",""),
                "capital_terme":float(st.session_state.get("f_cap",0) or 0),
                **{f"{qk}":st.session_state.get(f"f_{qk}","Non") for qk in [f"q{i}" for i in range(1,8)]},
                **{f"{qk}d":st.session_state.get(f"f_{qk}d","") for qk in [f"q{i}" for i in range(1,8)]},
            ...
            try:
                conn=get_conn()
                conn.execute(f"INSERT INTO bulletins_bia ({','.join(data.keys())}) VALUES ({','.join(['?']*len(data))})",list(data.values()))
                conn.commit(); conn.close()
                for k in [k for k in st.session_state.keys() if k.startswith("f_")]: del st.session_state[k]
                st.session_state.pop("bia_prod",None); st.session_state.pop("bia_step",None)
        ...
        with st.expander(f"📋 Valider un brouillon ({len(brous)} en attente)"):
            for _,br in brous.iterrows():
                ...
                if cc2.button("✅",key=f"val_{br['id']}"):
                    c=get_conn(); c.execute("UPDATE bulletins_bia SET statut='Validé' WHERE id=?",(br["id"],)); c.commit(); c.close()
                    st.balloons(); st.rerun()
                if cc3.button("🗑️",key=f"del_{br['id']}"):
                    c=get_conn(); c.execute("DELETE FROM bulletins_bia WHERE id=?",(br["id"],)); c.commit(); c.close(); st.rerun()
    # Filtres
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    conn_e=get_conn(); df_bia_e=pd.read_sql("SELECT * FROM bulletins_bia",conn_e); conn_e.close()
